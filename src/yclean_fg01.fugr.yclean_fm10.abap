...
*"----------------------------------------------------------------------
  ...
  _logger->add_success( iv_msgid = mc_msg-id iv_msgno = '002' iv_msgv1 = TEXT-t13 iv_msgv2 = |Yıl:{ VALUE #( iv_jahrper[ 1 ]-jahrper(4) OPTIONAL ) };PB türü:{ iv_curtp }| ).
  _logger->post( iv_refresh = abap_true ).
  TRY.
      yclean_cl03=>_rb2_rundat(
        EXPORTING
    ...
    CATCH cx_amdp_error INTO DATA(lv_exp).
      ...
      _logger->add_error( iv_msgid = mc_msg-id iv_msgno = '005' iv_msgv1 = TEXT-t13 iv_msgv2 = _msg+0(50) iv_msgv3 = _msg+50(50) iv_msgv4 = _msg+100(50) ).
  ENDTRY.
  _logger->post( iv_refresh = abap_true ).
