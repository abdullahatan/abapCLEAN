...
*->Commit Begin Of -aatan/11.03.2022 {
...
*       ev_result = 'Your entried value is not valid.';
*    END IF;
*->Commit End Of -aatan/11.03.2022 }

*->Begin Of -aatan/11.03.2022 {
     ev_rowcnt = :numberofentities;
     DELETE FROM acdoca as db WHERE( db.rclnt, db.rldnr, db.rbukrs, db.gjahr, db.belnr )
                                 IN ( SELECT DISTINCT db.rclnt, rldnr, rbukrs, gjahr, belnr
                                        FROM acdoca
                                        WHERE rclnt  = Session_context('CLIENT')
                                          AND rldnr  = :iv_rldnr
                                          AND rbukrs = :iv_bukrs
