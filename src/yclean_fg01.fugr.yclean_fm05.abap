...
*"--------------------------------------------------------------------
  ...
  _logger->add_success( iv_msgid = mc_msg-id iv_msgno = '002' iv_msgv1 = TEXT-t05 iv_msgv2 = |{ TEXT-p01 }:{ iv_bukrs }| ).
  _logger->post( iv_refresh = abap_true ).
  TRY.
      yclean_cl01=>_rb5_rundat(
        EXPORTING
          iv_bukrs  = iv_bukrs
          iv_afabe  = iv_afabe
        IMPORTING
    ...
    CATCH cx_amdp_error INTO DATA(lv_exp).
      _logger->add_error( iv_msgid = mc_msg-id iv_msgno = '005' iv_msgv1 = TEXT-t05 iv_msgv2 = lv_exp->get_text( ) ).
  ENDTRY.
  _logger->post( iv_refresh = abap_true ).
